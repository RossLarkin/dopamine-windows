    ...
    public class LyricsControlViewModel : ContextMenuViewModelBase
    {
        ...
        private async void RefreshLyricsAsync(TrackViewModel track)
        {
            ...
            try
            {
                ...
                if (mustDownloadLyrics)
                {
                    ...
                    {
                        Tracer.Error("Could not get lyrics online {0}. Exception: {1}", track.Path, ex.Message);
                    }
            ...
            {
                this.IsDownloadingLyrics = false;
                Tracer.Error("Could not show lyrics for Track {0}. Exception: {1}", track.Path, ex.Message);
                this.ClearLyrics(track);
                return;
            }
        ...
        private async Task HighlightLyricsLineAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    Tracer.Error("Could not highlight the lyrics. Exception: {0}", ex.Message);
                }
